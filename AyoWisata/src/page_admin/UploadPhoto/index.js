...
import { colors, fonts, showError, storeData, getData, useForm } from '../../utils';
...
const UploadPhoto = ({navigation, route}) => {
  ...
  const requestAccessLocation = async () => {
    try {
      ...
      if(cekPermission == PermissionsAndroid.RESULTS.GRANTED) {
        ...
        //   { enableHighAccuracy: true, maximumAge: 300, distanceFilter: 10, forceRequestLocation : true }
        // );
      }
    }
    catch(err){
  ...
  const uploadeAndContinue = () => {
    ...
    receiveData.Coordinate = currentPosition
    const newData = {
      latitude : currentPosition.latitude,
      longitude : currentPosition.longitude,
      next_form : 'MainApp_administrator',
      ...receiveData
    }
      ...
      <View style={styles.page}>
        <View style={styles.container}>
          <View style={styles.profile}>
            <Text style={styles.title}>Upload Photo Outlet</Text>
            <Gap height={30}/>
            <TouchableOpacity style={styles.avatarWrapper} onPress={getImage}>
              <Image source ={photo} style={styles.avatar}/>
              {hasPhoto && <IconRemovePhoto style = {styles.addPhoto}/> || <IconAddPhoto style = {styles.addPhoto}/>}
            </TouchableOpacity>
            <Text style={styles.name}>{receiveData.outletName}</Text>
            <Text style={styles.profession}>{receiveData.outletAddress}</Text>
          </View>
          <View>
